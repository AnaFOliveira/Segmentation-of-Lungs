...
def write_image_data_to_file(image_files, data_storage, truth_storage, image_shape, n_channels, affine_storage,
    ...
    i=0
    for num,set_of_files in enumerate(image_files):
        print(num+1,' of ', len(image_files), ' ', num*100/len(image_files),'%')
        

        images, affines = reslice_image_set(set_of_files, image_shape, label_indices=len(set_of_files) - 1, crop=crop)
        subject_data = [image for image in images]
        add_data_to_storage(data_storage, truth_storage, affine_storage, subject_data, affines[0], n_channels, 
                            truth_dtype)
        i=i+1
