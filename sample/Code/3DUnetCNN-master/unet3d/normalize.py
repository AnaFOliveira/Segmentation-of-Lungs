...
def reslice_image_set(in_files, image_shape, out_files=None, out_affines = None, label_indices=None, crop=False):
    if crop:
        crop_slices = get_cropping_parameters([in_files])
    else:
        crop_slices = None
    images, affines = read_image_files(in_files, image_shape=image_shape, crop=crop_slices, label_indices=label_indices)
    if out_files:
        for image, out_file in zip(images, out_files):
            image.to_filename(out_file)
        return [os.path.abspath(out_file) for out_file in out_files]
    if out_affines:
        for affine, out_file in zip(affines, out_affines):
            affine.to_filename(out_file)
        return [os.path.abspath(out_file) for out_file in out_affines]
    else:
        return images,affines


def get_complete_foreground(training_data_files):
    print("normalize, get_complete_foreground, training_data_files: ",training_data_files)
    ...
    return foreground.copy()
        #new_img_like(read_image(training_data_files[0][-1]), foreground)
