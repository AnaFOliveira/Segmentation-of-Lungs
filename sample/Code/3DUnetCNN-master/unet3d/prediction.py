...
def run_validation_case(data_index, output_dir, model, data_file, training_modalities,
    ...
    for i, modality in enumerate(training_modalities):
        ...
        image.to_filename(os.path.join(output_dir, "{0}_ct.nii.gz".format(modality)))

    test_truth = nib.Nifti1Image(data_file.root.truth[data_index][0], affine)
    test_truth.to_filename(os.path.join(output_dir, "{0}_truth.nii.gz"))
    ...
    if isinstance(prediction_image, list):
        for i, image in enumerate(prediction_image):
            image.to_filename(os.path.join(output_dir, "{0}_prediction.nii.gz".format(i + 1)))
    else:
        prediction_image.to_filename(os.path.join(output_dir, "{0}_prediction.nii.gz"))
...
def run_validation_cases(validation_keys_file, model_file, training_modalities, labels, hdf5_file,
    ...
    for index in validation_indices:
        if 'subject_ids' in data_file.root:
            ...
            case_directory = os.path.join(output_dir, "{}_validation_case".format(index))
